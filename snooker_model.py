...
function chk(id,fair){{
  ...
  if(v>fair){{
    ...
    b.style.background='#ff4444'; b.style.color='#fff';
    b.innerText='NO VALUE '+v+'<'+fair+' ('+edge.toFixed(1)+'%)';
  }}
}}
</script></head><body>
<h2>🎱 Snooker IQ V5.6.1 • 4 MARKETS</h2>
...
{m['p1']} {r1:.1f}% → Fair {fair(r1)} <input id='r1_{idx}' placeholder='B365' oninput="chk('r1_{idx}',{fair(r1)})"><span id='r1_{idx}_b' class='badge'>Enter B365</span><br>
{m['p2']} {r2:.1f}% → Fair {fair(r2)} <input id='r2_{idx}' placeholder='B365' oninput="chk('r2_{idx}',{fair(r2)})"><span id='r2_{idx}_b' class='badge'>Enter B365</span>

<div class='sec'>2. Match Win (First to 4)</div>
{m['p1']} {w1:.1f}% → Fair {fair(w1)} <input id='w1_{idx}' placeholder='B365' oninput="chk('w1_{idx}',{fair(w1)})"><span id='w1_{idx}_b' class='badge'>Enter B365</span><br>
{m['p2']} {w2:.1f}% → Fair {fair(w2)} <input id='w2_{idx}' placeholder='B365' oninput="chk('w2_{idx}',{fair(w2)})"><span id='w2_{idx}_b' class='badge'>Enter B365</span>

<div class='sec'>3. Correct Score 4-2</div>
{m['p1']} 4-2 {cs1:.1f}% → Fair {fair(cs1)} <input id='c1_{idx}' placeholder='B365' oninput="chk('c1_{idx}',{fair(cs1)})"><span id='c1_{idx}_b' class='badge'>Enter B365</span><br>
{m['p2']} 4-2 {cs2:.1f}% → Fair {fair(cs2)} <input id='c2_{idx}' placeholder='B365' oninput="chk('c2_{idx}',{fair(cs2)})"><span id='c2_{idx}_b' class='badge'>Enter B365</span>

<div class='sec'>4. Over 5.5 Frames</div>
Over 5.5 {ov:.1f}% → Fair {fair(ov)} <input id='o_{idx}' placeholder='B365' oninput="chk('o_{idx}',{fair(ov)})"><span id='o_{idx}_b' class='badge'>Enter B365</span><br>
